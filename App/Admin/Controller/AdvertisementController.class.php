...
function adslist(){      //项目管理页     
    ...
    $newsList_info = $newsList->where($where)->order('ads_sort')->limit($page->firstRow.','.$page->listRows)->select();
	Load('extend');
...
function addadvert(){      //添加项目      
	  ...
	  $news=M('advertisement');	 
	   ...
	   if($_POST['fid']){
			...
			$data['ad_content'] 	= $_POST['ad_content'];	
			$data['ad_econtent'] 	= $_POST['ad_econtent'];	
			
			$data["ad_update"]=date('Y-m-d',time());		
	    //图片上传		
		$info = $this->upload('myad/');//商品图片保存在Uploads/goods/目录
        //dump($info); exit;
