...
span[plugin="plugin"]{color: red; display: flex;justify-content: center;}
</style>
</head>
...
<div class="about_box">
	...
	<div class="about_right">
        ...
        <h2>网上报名</h2>
<form class="form form-inline"  method="post" id="applicaion_form">
<table width=100% >
<tbody>
<tr ><td width=15% align="right" class="div_form">姓 名：</td>
<td width="86%" class="right"><input type="text" id="t1" name="Name" class="form-control input-sm col-sm-3"></td>
</tr>
...
<input type="text" name="ConsultationDate" data-date-format="yyyy-mm-dd" class="date-picker" />
</td>
</tr>
<tr>
...
<td class="right"><input type="text" id="t3" name="tel" class="col-sm-3 form-control"></td>
</tr>
<tr ><td width=15% align="right" class="div_form">Q Q：</td>
<td class="right"><input type="text" id="t4" name="qq" class="col-sm-3 form-control"></td>
</tr>
<tr ><td width=15% align="right" class="div_form">邮箱：</td>
<td class="right"><input type="text" id="t5" name="email" class="col-sm-3 form-control"></td>
</tr>
<tr ><td width=15% align="right" class="div_form">微 信：</td>
<td class="right"><input type="text" id="t6" name="wechat" class="col-sm-3 form-control"></td>
</tr>
...
<td><button id="Submit1" type="button" class="btn btn-info" >立即报名</button> 
<!--基于bootsrap的弹出框-->
...
$(document).ready(function(){
    ...
    if (info) {
        if (!element.next("span[plugin=plugin]").length) {
            element.after("<span plugin='plugin'>"+info+"</span>");


        } else {
            element.next("span[plugin=plugin]").text(info);

        }
    }
 else {
    if (element.next("span[plugin=plugin]")) {
        element.next("span[plugin=plugin]").remove();
...
function checktel(tel){
	var regexp=/^1[3|4|5|7|8][0-9]{9}$/;
	 var res=regexp.test(tel);
	 return res;
}
//检查QQ
function checkqq(qq){
	var regexp=/^\d{5,10}$/;
	 var res=regexp.test(qq);
	 return res;
}
function checkwechat(wechat){
	var regexp=/^\w+$/;
	 var res=regexp.test(wechat);
	 return res;
}
function checkremark(remark){
	var regexp=/^\w+$/;
	 var res=regexp.test(remark);
	 return res;
}
...
//v
	$("input[name=Name]").change(function(){
	 var name=$(this).val();
	 var element=$(this);
	 if(name.length>10){alertinfo("名字长度不能超过10",element);determin=false}
	 	else if(name.length==0){alertinfo("请输入名字",element);determin=false}
	...
	$("input[name=email]").change(function(){
	 ...
	 if(!res){alertinfo("邮件格式不对",element);determin=false}
     else{alertinfo("",element)}
	...
	$("input[name=tel]").change(function(){
	 ...
	 if(!res){alertinfo("电话号码格式不对",element);determin=false}
     else{alertinfo("",element)}
	...
	$("input[name=wechat]").change(function(){
	 var wechat=$(this).val();
	 var element=$(this);
	 var res=checkwechat(wechat);
	 if(!res){alertinfo("请输入微信号",element);determin=false}
     else{alertinfo("",element)}
	...
	$("textarea[name=remark]").change(function(){
	 var remark=$(this).val();
	 var element=$(this);
	 var res=checkremark(remark);
	 if(!res){alertinfo("说点什么吧~~",element);determin=false}
     else{alertinfo("",element)}
	...
	$("#Submit1").click(function(){
		...
		if(!determin){
			fuzzyalert("请检查信息是否正确")
			return false;
		}
        var elements=$("input[name='course[]']").not("checkbox");
